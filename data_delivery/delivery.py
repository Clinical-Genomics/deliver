...
def getsamplesfromflowcell(demuxdir, flwc):
    ...
    for sample in samples:
        ...
        # remove reprep (B) and reception control fail (F) letters
...
def getsampleinfofromname(pars, sample):
    ...
    with db.create_tunnel(pars['TUNNELCMD']):
        with db.dbconnect(pars['CLINICALDBHOST'], pars['CLINICALDBPORT'],
                          pars['STATSDB'], pars['CLINICALDBUSER'],
                          pars['CLINICALDBPASSWD']) as dbc:
            replies = dbc.generalquery(query)
    return replies


def make_link(demuxdir, outputdir, family_id, cust_name, sample_name, fclane):
    fastqfiles = glob.glob("{demuxdir}*{fc}*/Unalign*/Project_*/Sample_{sample_name}"
                           "_*/*L00{lane}*gz"
                           .format(demuxdir=demuxdir, fc=fclane['fc'],
                                   sample_name=sample_name, lane=fclane['lane']))
    fastqfiles.extend(glob.glob("{demuxdir}*{fc}*/Unalign*/Project_*/Sample_"
                                "{sample_name}[BF]_*/*L00{lane}*gz"
                                .format(demuxdir=demuxdir, fc=fclane['fc'],
                                        sample_name=sample_name, lane=fclane['lane'])))
    ...
    for fastqfile in fastqfiles:
        ...
        flowcell = rundir[-9:]
        newname = ("{lane}_{date}_{fc}_{sample_name}_{index}_{readdirection}.fastq.gz"
                   .format(lane=nameparts[3][-1:], date=date, fc=flowcell,
                           sample_name=sample_name, index=nameparts[2],
                           readdirection=nameparts[4][-1:]))
        try:
            os.symlink(fastqfile, os.path.join(outputdir, 'exomes', sample_name,
                       'fastq', newname))
        except:
    ...
    for sample in smpls.iterkeys():
        ...
        with lims.limsconnect(params['apiuser'], params['apipass'], params['baseuri']) as lmc:
            ...
            if analysistype is None:
                print("WARNING: Sequencing Analysis tag not defined for {}"
                      .format(sample))
                # skip to the next sample
                continue

            if analysistype == 'RML': # skip Ready Made Libraries
                print("WARNING: Ready Made Library. Skipping link creation for {}"
                      .format(sample))
                continue

            # Accepted readcount is 75% of ordered million reads
            readcounts = .75 * float(analysistype[-3:])
            ...
            if not re.match(r'cust\d{3}', cust_name):
                print("WARNING '{}' does not match an internal customer name"
                      .format(cust_name))
                cust_name = None
            if cust_name == None:
        ...
        # counter for total readcount of sample
        read_count = 0
        # list to keep flowcell names and lanes for a sample
        fclanes = []
        for info in dbinfo:
            # Use readcount from lane only if it satisfies QC [=80%]
            if info['q30'] > 80:
                read_count += info['M_reads']
                fclanes.append(dict(((key, info[key]) for key in ['fc', 'q30', 'lane'])))

        if readcounts:
            # If enough reads are obtained do
            if read_count > readcounts:
                print("{sample} Passed {readcount} M reads\nUsing reads from {fclanes}"
                      .format(sample=sample, readcount=read_count, fclanes=fclanes))
                ...
                if cust_name != None and family_id != None:
                    try:
                        os.makedirs(os.path.join(outputdir, cust_name, family_id,
                                                 'exomes', sample, 'fastq'))
                    except OSError:
                        pass
                ...
                for fclane in fclanes:
                    make_link(demuxdir=params['DEMUXDIR'], outputdir=outputdir,
                              family_id=family_id, cust_name=cust_name,
                              sample_name=sample, fclane=fclane)

            # Otherwise just present the data
            else:
                print("{sample} FAIL with {readcount} M reads.\n"
                      "Requested with {reqreadcount} M reads.\n"
                      "These flowcells summarized {fclanes}"
                      .format(sample=sample, readcount=read_count, fclanes=fclanes,
                              reqreadcount=readcounts))
        else:
