...
def main(run_dir):
    ...
    samples = get_sample_paths(run_dir)
    ...
    for sample, sample_path in samples.items():
        with lims.limsconnect(params['apiuser'], params['apipass'],
            ...
            if kit_type == 'QXT':
                ...
                with open("{}/trimming.txt".format(run_dir), 'w') as t_file:
    ...
    # trimmed.txt file
    if sbatch_ids:
        launch_end(trim_indir=fastq_trim_dir, base_dir=run_dir, sbatch_ids=sbatch_ids)
